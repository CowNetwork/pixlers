...
class DrawPhaseWorldProvider(game: SpigotGame, config: WorldProviderConfig) : WorldProvider(game, config) {
    ...
    override fun selectWorld(): World {
        ...
        Paths.get("/opt/spigot/plugins/pixlers/maps/pixlers-cinema").toFile().copyRecursively(Paths.get(this.game.config.workingDirectory, targetName).toFile(), overwrite = true)
